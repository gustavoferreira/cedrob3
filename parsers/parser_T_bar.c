...
static void opts_init(Options *o){
    memset(o, 0, sizeof(*o));
    o->symbols[0] = 0;
    strncpy(o->symbols, "WING26,WDOF26", sizeof(o->symbols)-1);
    o->sleep_sec = 0.25;
    o->bar_sec = 1;
...
static void usage(const char *prog){
    fprintf(stderr,
        ...
        "  --follow (tail -f)\n"
        "  --sleep-sec 0.25\n"
        "  --bar-sec 1        (agrega e grava a cada N segundos; ex: 5)\n"
...
static int find_symbol(SymSlot *slots, int n, const char *sym){
    for(int i=0;i<n;i++){
        if(strcmp(slots[i].name, sym)==0) return i;
    }
    return -1;
}
...
int main(int argc, char **argv){
    ...
    while(1){
        ...
        while(have_current_dt && current_dt < dt_sec){
            if(opt.bar_sec <= 1 || ((long long)current_dt % opt.bar_sec) == 0){
                flush_second(current_dt, slots, nslots, fout, sess_start, sess_end, sess_enabled, &opt);
            }
            current_dt += 1;
        }
